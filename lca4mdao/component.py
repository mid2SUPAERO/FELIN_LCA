...
def _setup_procs(self, pathname, comm, prob_meta):
    ...
    for name, param in MdaoParameter.load().items():
        self.add_input(param["mdao_name"], val=param["amount"], tags='lca', units=param["units"])
    self.declare_partials(['*'], ['*'], method='fd')

'''def setup_procs(self, pathname, comm, prob_meta):
        super().setup_procs(pathname, comm, prob_meta)
          check dependency chain if multiple LCA modules
        for name, param in MdaoParameter.load().items():
            self.add_input(param["mdao_name"], val=param["amount"], tags=['lca'], units=param["units"])
        self.declare_partials(['*'], ['*'], method='fd')
          partials'''
