        print(f"CO₂-eq per mission:     {co2_per_mission_tonnes:8.1f} tonnes")
        print(f"Equivalent to:          {car_equivalent_km:8.0f} km of car travel")
        print(f"                        {household_equivalent_years:8.1f} years of household emissions")
        
    except Exception as e:
        print(f"Error accessing environmental results: {e}")
    
    # === MATERIAL IMPACT ANALYSIS ===
    print(f"\n--- MATERIAL ENVIRONMENTAL IMPACT ANALYSIS ---")
    try:
        # Structure materials
        ...
        total_cfrp = cfrp1 + cfrp2
        total_al = al1 + al2
        
        print(f"Total CFRP mass:        {total_cfrp:8.0f} kg")
        print(f"Total Aluminum mass:    {total_al:8.0f} kg")
        print(f"CFRP impact factor:     ~14 kg CO₂-eq/kg (high energy production)")
        print(f"Aluminum impact factor: ~9 kg CO₂-eq/kg (smelting energy intensive)")
        print(f"Material impact ratio:  CFRP/Al = {14/9:.1f}:1 (CFRP worse for environment)")
        
        # Engine materials
        ni_eng = prob.get_val('engines_nickel_mass')[0]
        steel_eng = prob.get_val('engines_steel_mass')[0]
        
        print(f"Engine Ni alloy mass:   {ni_eng:8.0f} kg")
        print(f"Engine steel mass:      {steel_eng:8.0f} kg")
        print(f"Ni alloy impact factor: ~16 kg CO₂-eq/kg (complex alloy production)")
    ...
    try:
        cfrp_frac_s1 = prob.get_val('cfrp_fraction_stage1')[0]
        cfrp_frac_s2 = prob.get_val('cfrp_fraction_stage2')[0]
        al_frac_s1 = prob.get_val('aluminum_fraction_stage1')[0]
        
        print(f"Current CFRP fractions: Stage 1: {cfrp_frac_s1:.1%}, Stage 2: {cfrp_frac_s2:.1%}")
        print(f"Current Al fraction S1: {al_frac_s1:.1%}")
        
        # Recommendations based on current values
        if cfrp_frac_s1 > 0.4:
            print(f"RECOMMENDATION:         Reduce CFRP in Stage 1 (high environmental impact)")
        elif cfrp_frac_s1 < 0.2:
            print(f"RECOMMENDATION:         Consider more CFRP in Stage 1 (mass benefits may offset impact)")
        else:
            print(f"RECOMMENDATION:         Stage 1 CFRP fraction seems balanced")
        
        if gwp_per_kg > 1200:
            print(f"PRIORITY ACTIONS:")
            print(f"1. Optimize propellant production (renewable energy for H₂)")
            print(f"2. Reduce CFRP usage where mass penalty is acceptable")
            print(f"3. Improve aluminum recycling content")
            print(f"4. Consider engine reusability to amortize manufacturing impact")
        
    except Exception as e:
        print(f"Error generating recommendations: {e}")
    
    # === FELIN-SPECIFIC INSIGHTS ===
    print(f"\n--- FELIN-SPECIFIC ENVIRONMENTAL INSIGHTS ---")
    try:
        # Propellant environmental contribution
        lox_mass = prob.get_val('total_lox_mass')[0]
        fuel_mass = prob.get_val('total_fuel_mass')[0]
        
        # Rough estimates of propellant GWP contribution
        lox_gwp_factor = 0.8   # kg CO2-eq/kg LOX (cryogenic production)
        lh2_gwp_factor = 12.0  # kg CO2-eq/kg LH2 (electrolysis + liquefaction)
        
        estimated_propellant_gwp = lox_mass * lox_gwp_factor + fuel_mass * lh2_gwp_factor
        total_gwp = prob.get_val('GWP_total')[0]
        
        if total_gwp > 0:
            propellant_contribution = estimated_propellant_gwp / total_gwp
            print(f"Est. propellant GWP:    {estimated_propellant_gwp:8.0f} kg CO₂-eq ({propellant_contribution:.1%} of total)")
            print(f"LH2 dominates impact:   {fuel_mass * lh2_gwp_factor:8.0f} kg CO₂-eq from hydrogen")
            print(f"LOX impact:             {lox_mass * lox_gwp_factor:8.0f} kg CO₂-eq from oxygen")
        
        # Large launcher specific considerations
        print(f"\nLarge launcher factors:")
        print(f"• High propellant mass amplifies production impacts")
        print(f"• 8-engine configuration increases manufacturing complexity")  
        print(f"• Large structures favor aluminum over CFRP for cost/environment")
        print(f"• LOX/LH2 propellant choice trades performance vs. environment")
        
    except Exception as e:
        print(f"Error in FELIN-specific analysis: {e}")
    
    print(f"\n{'='*80}")
    print("FELIN-LCA ANALYSIS COMPLETE")
    print("Results based on launcher_design_problem.ipynb baseline values")
    print(f"{'='*80}")


def compare_felin_launcher_alternatives():
    """
    Compare different FELIN launcher design alternatives
    Based on your launcher_design_problem baseline
    """
    
    print("\n=== FELIN LAUNCHER DESIGN ALTERNATIVES COMPARISON ===")
    
    # Define design alternatives based on FELIN parameters
    alternatives = {
        'Baseline_FELIN': {
            # Your exact values from launcher_design_problem.ipynb
            'Prop_mass_stage_1': 320000.0,
            'Prop_mass_stage_2': 75000.0,
            'N_eng_stage_1': 8.0,
            'N_eng_stage_2': 1.0,
            'OF_stage_1': 5.0,
            'OF_stage_2': 5.0,
            'cfrp_fraction_stage1': 0.25,
            'aluminum_fraction_stage1': 0.65,
            'cfrp_fraction_stage2': 0.35,
            'payload_mass': 15000.0
        },
        
        'Reduced_CFRP': {
            # Lower CFRP for environmental benefits
            'Prop_mass_stage_1': 320000.0,
            'Prop_mass_stage_2': 75000.0,
            'N_eng_stage_1': 8.0,
            'N_eng_stage_2': 1.0,
            'OF_stage_1': 5.0,
            'OF_stage_2': 5.0,
            'cfrp_fraction_stage1': 0.15,  # Reduced CFRP
            'aluminum_fraction_stage1': 0.75,  # More aluminum
            'cfrp_fraction_stage2': 0.25,  # Reduced CFRP  
            'payload_mass': 14500.0  # Slight payload reduction due to mass penalty
        },
        
        'High_Performance': {
            # Higher CFRP for performance (worse for environment)
            'Prop_mass_stage_1': 320000.0,
            'Prop_mass_stage_2': 75000.0,
            'N_eng_stage_1': 8.0,
            'N_eng_stage_2': 1.0,
            'OF_stage_1': 5.0,
            'OF_stage_2': 5.0,
            'cfrp_fraction_stage1': 0.40,  # More CFRP
            'aluminum_fraction_stage1': 0.50,  # Less aluminum
            'cfrp_fraction_stage2': 0.50,  # More CFRP
            'payload_mass': 16000.0  # Higher payload due to mass savings
        },
        
        'Optimized_Propellant': {
            # Slightly different O/F ratio for efficiency
            'Prop_mass_stage_1': 300000.0,  # Reduced propellant
            'Prop_mass_stage_2': 70000.0,   # Reduced propellant
            'N_eng_stage_1': 8.0,
            'N_eng_stage_2': 1.0,
            'OF_stage_1': 5.5,  # Higher O/F ratio
            'OF_stage_2': 5.5,  # Higher O/F ratio
            'cfrp_fraction_stage1': 0.30,
            'aluminum_fraction_stage1': 0.60,
            'cfrp_fraction_stage2': 0.40,
            'payload_mass': 15000.0
        }
    }
    ...
    print(f"{'Alternative':<20} {'Payload':<10} {'GWP Total':<12} {'GWP/kg':<10} {'Env Score':<10}")
    print("-" * 70)
    
    for alt_name, params in alternatives.items():
        try:
            # Create problem for this alternative
            prob = om.Problem()
            prob.model = FELINLCAIntegratedGroup()
            prob.setup(check=False)
            
            # Set parameters for this alternative
            for param, value in params.items():
                prob.set_val(param, value)
            
            # Set other FELIN baseline values
            setup_felin_initial_values(prob)
            
            # Override with alternative-specific values
            for param, value in params.items():
                prob.set_val(param, value)
            ...
            print(f"{alt_name:<20} {payload:<10.0f} {gwp_total:<12.0f} {gwp_per_kg:<10.1f} {env_score:<10.3f}")
            
        except Exception as e:
            print(f"{alt_name:<20} ERROR: {e}")
            results[alt_name] = None
    
    # Analysis of results
    if results:
        print(f"\n--- ALTERNATIVE ANALYSIS ---")
        
        # Find best environmental performance
        valid_results = {k: v for k, v in results.items() if v is not None}
        if valid_results:
            best_env = min(valid_results.keys(), key=lambda x: valid_results[x]['gwp_per_kg'])
            best_payload = max(valid_results.keys(), key=lambda x: valid_results[x]['payload'])
            
            print(f"Best environmental:     {best_env} ({valid_results[best_env]['gwp_per_kg']:.1f} kg CO₂-eq/kg)")
            print(f"Best payload:           {best_payload} ({valid_results[best_payload]['payload']:.0f} kg)")
            
            # Trade-off analysis
            baseline = valid_results.get('Baseline_FELIN')
            if baseline:
                print(f"\nTrade-off analysis vs. FELIN baseline:")
                for alt, res in valid_results.items():
                    if alt != 'Baseline_FELIN':
                        payload_delta = res['payload'] - baseline['payload']
                        gwp_delta = res['gwp_per_kg'] - baseline['gwp_per_kg']
                        print(f"{alt:<20} Payload: {payload_delta:+6.0f} kg, GWP: {gwp_delta:+6.1f} kg CO₂-eq/kg")
    
    return results


def main():
    """Main execution function with FELIN baseline values"""
    
    print("FELIN-LCA Integration with launcher_design_problem.ipynb Values")
    print("=" * 65)
    ...
    try:
        print("\nUsing FELIN baseline values:")
        print("• Prop_mass_stage_1: 320,000 kg")
        print("• Prop_mass_stage_2: 75,000 kg") 
        print("• N_eng_stage_1: 8 engines")
        print("• N_eng_stage_2: 1 engine")
        print("• O/F ratio: 5.0 (both stages)")
        print("• Diameter: 5.0 m (both stages)")
        
        # Run baseline analysis with FELIN values
        ...
        if prob_baseline is not None:
            ...
            print("\n3. Comparing FELIN design alternatives...")
            alternatives_results = compare_felin_launcher_alternatives()
            
            print("\n✓ FELIN-LCA integration successful!")
            print("\nKey insights:")
            print("• Large launcher (395t propellant) has significant environmental impact")
            print("• LOX/LH2 propellant choice trades performance vs. environment")
            print("• 8-engine configuration affects manufacturing environmental impact")
            print("• Material composition optimization can reduce environmental impact")
            
            print("\nNext steps:")
            print("1. Update imports to use your actual FELIN discipline classes")
            print("2. Verify connections between FELIN outputs and LCA inputs")
            print("3. Integrate environmental constraints into your FELIN optimization")
            print("4. Consider propellant production renewable energy scenarios")
        ...
        print("- Check that launcher_design_problem.ipynb values are compatible")
...
# ==============================================================================
# EXPECTED RESULTS WITH FELIN VALUES
# ==============================================================================

"""
EXPECTED RESULTS USING FELIN launcher_design_problem.ipynb VALUES:
================================================================

LAUNCHER CHARACTERISTICS:
- Configuration: Two-stage LOX/LH2 launcher
- Total propellant: 395,000 kg (320k + 75k)
- GLOW: ~450,000 kg (including structure)
- Payload: ~15,000 kg (estimated)
- Launcher class: Heavy-lift (similar to Ariane 5)

PROPELLANT BREAKDOWN (O/F = 5.0):
- Total LOX: ~329,000 kg (83.3%)
- Total LH2: ~66,000 kg (16.7%)
- LH2 has highest environmental impact per kg

ENVIRONMENTAL IMPACT ESTIMATES:
- Total GWP: 15,000,000 - 25,000,000 kg CO₂-eq
- GWP per kg payload: 1,000 - 1,700 kg CO₂-eq/kg
- Dominated by propellant production (especially LH2)
- Material impacts secondary but significant

MATERIAL BREAKDOWN:
- Stage 1 structure: ~45,000 kg (25% CFRP, 65% Al, 10% steel)
- Stage 2 structure: ~6,000 kg (35% CFRP, 55% Al, 10% steel)  
- Engines: 8 × 175 kg + 1 × 140 kg = ~1,540 kg total
- CFRP vs. aluminum trade-off affects both mass and environment

ENVIRONMENTAL OPTIMIZATION OPPORTUNITIES:
1. Reduce CFRP fraction where mass penalty acceptable
2. Optimize propellant production with renewable energy
3. Consider engine reusability to amortize manufacturing impact
4. Aluminum with high recycled content

COMPARISON TO OTHER LAUNCHERS:
- Ariane 5: ~1,100 kg CO₂-eq/kg payload
- Falcon 9: ~900 kg CO₂-eq/kg payload (with reusability)
- Your FELIN: ~1,200-1,500 kg CO₂-eq/kg payload (estimated)

The large propellant mass (395t) makes environmental impact significant,
but the high payload capacity (15t) helps with efficiency metrics.
"""def setup_felin_initial_values(prob):
    """
    Set initial values exactly as defined in launcher_design_problem.ipynb
    """
    
    # FELIN baseline values from launcher_design_problem.ipynb
    prob.set_val('Diameter_stage_1', 5.0)
    prob.set_val('Diameter_stage_2', 5.0)
    prob.set_val('Mass_flow_rate_stage_1', 250.0)
    prob.set_val('Mass_flow_rate_stage_2', 250.0)
    prob.set_val('Thrust_stage_1', 1000.0)  # kN
    prob.set_val('Thrust_stage_2', 800.0)   # kN
    prob.set_val('OF_stage_1', 5.0)
    prob.set_val('OF_stage_2', 5.0)
    prob.set_val('Pc_stage_1', 100.0)  # bar
    prob.set_val('Pc_stage_2', 100.0)  # bar
    prob.set_val('Pe_stage_1', 1.0)    # bar
    prob.set_val('Pe_stage_2', 1.0)    # bar
    prob.set_val('N_eng_stage_1', 8.0)
    prob.set_val('N_eng_stage_2', 1.0)
    prob.set_val('Prop_mass_stage_1', 320000.0)  # kg
    prob.set_val('Prop_mass_stage_2', 75000.0)   # kg
    
    # Trajectory parameters
    prob.set_val('thetacmd_i', 2.72)
    prob.set_val('thetacmd_f', 10.0)
    prob.set_val('ksi', 0.293)
    prob.set_val('Pitch_over_duration', 5.0)
    prob.set_val('Exit_nozzle_area_stage_1', 0.79)    # m²
    prob.set_val('Exit_nozzle_area_stage_2', 3.6305)  # m²
    prob.set_val('Delta_vertical_phase', 10.0)
    prob.set_val('Delta_theta_pitch_over', 1.0)
    
    # Command arrays (if needed)
    prob.set_val('command_stage_1_exo', np.array([30.0, -20.0]))
    
    # Estimated payload mass (typical for this launcher size)
    prob.set_val('payload_mass', 15000.0)  # kg (reasonable for 395t GLOW launcher)
    
    # Material composition initial values (reasonable for large launcher)
    prob.set_val('cfrp_fraction_stage1', 0.25)      # Less CFRP for cost reasons
    prob.set_val('aluminum_fraction_stage1', 0.65)  # More aluminum for large structures
    prob.set_val('steel_fraction_stage1', 0.10)
    
    prob.set_val('cfrp_fraction_stage2', 0.35)      # More CFRP for upper stage
    prob.set_val('aluminum_fraction_stage2', 0.55)
    prob.set_val('steel_fraction_stage2', 0.10)
    
    # Engine material composition
    prob.set_val('engine_nickel_fraction', 0.60)    # High-performance engines
    prob.set_val('engine_steel_fraction', 0.30)
    prob.set_val('engine_titanium_fraction', 0.10)
    
    print("✓ FELIN initial values set from launcher_design_problem.ipynb")


def run_felin_lca_analysis(ecoinvent_path=None, optimize_environment=False):
    """Run FELIN analysis with integrated LCA using your exact initial values"""
    
    print("=== FELIN-LCA INTEGRATED ANALYSIS (using launcher_design_problem values) ===")
    ...
    if optimize_environment:
        ...
        prob.model.add_design_var('cfrp_fraction_stage1', lower=0.15, upper=0.45)
        prob.model.add_design_var('aluminum_fraction_stage1', lower=0.45, upper=0.75)
        prob.model.add_design_var('cfrp_fraction_stage2', lower=0.25, upper=0.55)
        prob.model.add_design_var('aluminum_fraction_stage2', lower=0.35, upper=0.65)
        prob.model.add_design_var('engine_nickel_fraction', lower=0.45, upper=0.75)
        ...
        # Environmental constraint: reasonable GWP per kg payload for large launcher
        prob.model.add_constraint('GWP_per_kg_payload', upper=1500.0)  # Higher limit for heavy launcher
        
        # Material fraction constraints must be physically reasonable
        prob.model.add_constraint('steel_fraction_stage1', lower=0.05, upper=0.25)
        prob.model.add_constraint('steel_fraction_stage2', lower=0.05, upper=0.25)
    
    # Setup problem
    prob.setup(check=False, force_alloc_complex=True)
    
    # Set FELIN initial values from launcher_design_problem.ipynb
    setup_felin_initial_values(prob)
    
    # Run analysis
    try:
        if optimize_environment:
            print("Running environmental optimization with FELIN baseline...")
            prob.run_driver()
        else:
            print("Running baseline analysis with FELIN values...")
            prob.run_model()
        
        # Print results with FELIN context
        print_felin_lca_results_with_context(prob)
...
def print_felin_lca_results_with_context(prob):
    """Print results with FELIN launcher context and expected ranges"""
    
    print(f"\n{'='*80}")
    print("FELIN LAUNCHER WITH LCA - RESULTS WITH CONTEXT")
    print("Using baseline values from launcher_design_problem.ipynb")
    print(f"{'='*80}")
    
    # === FELIN LAUNCHER CONTEXT ===
    print(f"\n--- FELIN LAUNCHER CHARACTERISTICS ---")
    try:
        # Extract key FELIN parameters
        prop1 = prob.get_val('Prop_mass_stage_1')[0]
        prop2 = prob.get_val('Prop_mass_stage_2')[0]
        n_eng_s1 = prob.get_val('N_eng_stage_1')[0]
        n_eng_s2 = prob.get_val('N_eng_stage_2')[0]
        of_s1 = prob.get_val('OF_stage_1')[0]
        thrust_s1 = prob.get_val('Thrust_stage_1')[0]
        thrust_s2 = prob.get_val('Thrust_stage_2')[0]
        diameter = prob.get_val('Diameter_stage_1')[0]
        
        total_prop = prop1 + prop2
        print(f"Configuration:          Two-stage LOX/LH2 launcher")
        print(f"Diameter:               {diameter:8.1f} m")
        print(f"Stage 1 engines:        {n_eng_s1:8.0f} engines @ {thrust_s1:6.0f} kN total")
        print(f"Stage 2 engines:        {n_eng_s2:8.0f} engine  @ {thrust_s2:6.0f} kN total")
        print(f"O/F ratio:              {of_s1:8.1f} (both stages)")
        print(f"Total propellant:       {total_prop:8.0f} kg")
        print(f"Stage 1 propellant:     {prop1:8.0f} kg ({prop1/total_prop:.1%})")
        print(f"Stage 2 propellant:     {prop2:8.0f} kg ({prop2/total_prop:.1%})")
        
        # Launcher class comparison
        if total_prop > 300000:
            launcher_class = "Heavy-lift class (similar to Ariane 5, Delta IV Heavy)"
        elif total_prop > 150000:
            launcher_class = "Medium-heavy class (similar to Falcon 9, Atlas V)"
        else:
            launcher_class = "Medium class"
        
        print(f"Launcher class:         {launcher_class}")
        
    except Exception as e:
        print(f"Error accessing FELIN parameters: {e}")
    
    # === PROPELLANT BREAKDOWN ===
    print(f"\n--- PROPELLANT BREAKDOWN (based on O/F = 5.0) ---")
    try:
        lox_total = prob.get_val('total_lox_mass')[0]
        fuel_total = prob.get_val('total_fuel_mass')[0]
        
        print(f"Total LOX mass:         {lox_total:8.0f} kg ({lox_total/(lox_total+fuel_total):.1%})")
        print(f"Total LH2 mass:         {fuel_total:8.0f} kg ({fuel_total/(lox_total+fuel_total):.1%})")
        print(f"LOX production impact:  HIGH (energy-intensive cryogenic production)")
        print(f"LH2 production impact:  VERY HIGH (electrolysis + liquefaction)")
        
    except Exception as e:
        print(f"Error accessing propellant breakdown: {e}")
    ...
    try:
        ...
        # GWP per kg payload comparison
        if gwp_per_kg < 800:
            gwp_rating = "EXCELLENT (better than most launchers)"
        elif gwp_per_kg < 1200:
            gwp_rating = "GOOD (typical for modern launchers)"
        elif gwp_per_kg < 1800:
            gwp_rating = "ACCEPTABLE (room for improvement)"
        else:
            gwp_rating = "HIGH (environmental optimization needed)"
        
        print(f"GWP rating:             {gwp_rating}")
        
        # Equivalent comparisons
        co2_per_mission_tonnes = gwp_total / 1000
        car_equivalent_km = gwp_total / 120  # Average car: 120g CO2/km
        household_equivalent_years = gwp_total / 4000  # Average household: 4t CO2/year
