...
def apply_openmdao_patch():
    """Make lca4mdao LcaCalculationComponent compatible with newer OpenMDAO."""
    try:
        import inspect
        ...
        if 'prob_meta' not in sig.parameters:
            ...
            def _setup_procs_compat(self, pathname, comm, prob_meta):
                # call original 3-arg version
                return _orig(self, pathname, comm)

            _LCC._setup_procs = _setup_procs_compat
            print("Patched LcaCalculationComponent._setup_procs for OpenMDAO compatibility.")
    except Exception as e:
        print("apply_openmdao_patch() failed or not needed:", e)
...
class FELINEnvironmentalDiscipline(ExplicitComponentLCA):
    ...
    def setup(self):
        ...
        self.add_output('launch_operations_electricity', val=0.0, units='unitless',
                       ...
                       lca_name='launch_operations_electricity')
...
class FELINLCACalculation(LcaCalculationComponent):
    """Strict LCA calc: requires LCIA methods; never fabricates values."""

    def setup(self):
        # ---- inputs from materials/ops ----
        for n in [
        ...
        # Foreground groups created in your helper DB
        group_dict = {
        ...
        # ---- ESA category methods (all required) ----
        self.ESA_methods = {}
        missing = []
        for code, options in ESA_METHOD_OPTIONS.items():
            ...
            # create true LCA-backed outputs (no plain add_output here)
            self.add_lca_output(f'ESA_{code}', group_dict, method_key=method, units='unitless')

        if missing:
            raise RuntimeError(
                f"Missing required ESA LCIA methods: {missing}. "
                "Import EF 3.0/CML/USEtox methods into this Brightway project."
            )

        # ---- Primary GWP (required) ----
        gwp_m = _find_first_matching([('ipcc 2013','climate change','gwp 100a')])
        if gwp_m is None:
            raise RuntimeError("Required LCIA method 'IPCC 2013 GWP 100a' not available.")
        self.add_lca_output('GWP_total', group_dict, method_key=gwp_m, units='kg')

    def _setup_procs(self, pathname, comm, prob_meta=None):
        # keep the compatibility shim you already added
        parent = super(FELINLCACalculation, self)._setup_procs
        for args in ((pathname, comm, None, prob_meta),
                     (pathname, comm, prob_meta),
                     (pathname, comm)):
            try:
                return parent(*args)
            except TypeError:
                pass
        from openmdao.core.explicitcomponent import ExplicitComponent as _OMExplicit
        try:
            return _OMExplicit._setup_procs(self, pathname, comm, prob_meta)
        except TypeError:
            return _OMExplicit._setup_procs(self, pathname, comm, None, prob_meta)

    def compute(self, inputs, outputs):
        # CRITICAL: call parent to push parameters & run real BW LCA
        return super(FELINLCACalculation, self).compute(inputs, outputs)
...
class FELINLCACalculation(LcaCalculationComponent):
    ...
    # inside class FELINLCACalculation(LcaCalculationComponent):
    def _setup_procs(self, pathname, comm, prob_meta=None):
        """Compatibility adapter for old/new OpenMDAO & lca4mdao signatures."""
        # 1) Try the parent (lca4mdao) with multiple possible signatures
        parent_method = super(FELINLCACalculation, self)._setup_procs
        for args in (
            (pathname, comm, None, prob_meta),  # old: (pathname, comm, mode, prob_meta)
            (pathname, comm, prob_meta),        # mid: (pathname, comm, prob_meta)
            (pathname, comm),                   # very old: (pathname, comm)
        ):
            try:
        ...
        # 2) Fall back to OpenMDAO's ExplicitComponent implementation
        from openmdao.core.explicitcomponent import ExplicitComponent as _OMExplicit
        try:
            # new OpenMDAO ExplicitComponent signature
            return _OMExplicit._setup_procs(self, pathname, comm, prob_meta)
        except TypeError:
            # older OpenMDAO ExplicitComponent signature
            return _OMExplicit._setup_procs(self, pathname, comm, None, prob_meta)
'''
...
class FELINEnvironmentalGroup(om.Group):
    def setup(self):
        ...
        self.add_subsystem(
            'materials', FELINEnvironmentalDiscipline(),
            promotes=['*']
        )

        # 2) LCA calculation (keep)
        self.add_subsystem(
        ...
        # 3) ESA single score (keep)
        self.add_subsystem(
            'esa_score', ESASingleScore(),
            promotes_inputs=[f'ESA_{c}' for c in ESA_WEIGHTS.keys()],
            promotes_outputs=['ESA_single_score']
        )

        # 4) Derived metrics (keep; make sure ESA_single_score is in promotes_inputs)
        self.add_subsystem(
...
def apply_openmdao_patch():
    # No-op now; handled by subclass adapters
    return None