...
class Trajectory_comp(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        z = inputs['Table_CX_complete_ascent']

        # Make sure z has shape (len(Table_incidence), len(Table_Mach_))
        if z.shape != (len(Table_incidence), len(Table_Mach_)):
            z = z.T
        
        Interp2_CX_complete_launcher=interpolate.RectBivariateSpline(Table_incidence,Table_Mach_,z)
